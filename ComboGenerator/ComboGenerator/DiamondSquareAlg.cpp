...
struct Point {
	...
	Point(float x, float y, float z, string url, string disName, string fileName, string PR) : 
		_x(x), _y(y), _z(z), _url(url), _disName(disName), _fileName(fileName), _pr(PR) { }
};
...
void initTerrain()
{
	...
	while(true) 
	{
		...
		if(temp.find("DisplayName") != temp.npos) {
			...
			int PRPos = earl.find_first_of("[");
			PR.replace(PR.find("PreReqs: "), 9, "");
			dFile.getline(line, 2000);
			...
			Point p(xp, yp, zp, url, toPushName, toPushFile, PR);
	...
	*/
	...
	{
		oFile << nodes[l]._disName.c_str() <<  ", " << nodes[l]._fileName.c_str() <<  ", " << nodes[l]._x << ", " << nodes[l]._y << ", " << nodes[l]._z << ", " << nodes[l]._url.c_str() << ", " << nodes[l]._pr.c_str() << endl;
	}
...
//int main(int argc, char** argv)
{
	...
	*/
	...
	string filenm = "coords.txt";
	ofstream oFile;
	oFile.open(filenm);

	cout << "Writing coordinates..." << endl;
	...
	for( int l = 0; l < nodes.size(); l++ )
	{
		...
		oFile << nodes[l].getPR().c_str() << endl;
...
int main(int argc, char** argv)
{
	cout << "LL/XXI Terrain Generator v3" << endl;
	cout << "Phase 1 Start" << endl;
	treeBuilder();
